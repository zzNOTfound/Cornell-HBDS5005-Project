...
## data wrangling
data <- df %>%
  select(genhlth, sleptim1, sex, menthlth, fruit1, fruitju1, avedrnk2)
data
```
## Research question 1
```{r}
...
ggplot(data2, aes(x= genhlth, y = avg_sleep)) + geom_point(aes(x= genhlth, y = avg_sleep)) + geom_line() + labs(x = "general health", y = "average sleep hours", title = "general health vs average sleep hours") + ylim(6.5,7.2)
```
...
```
## Research Question2:
Is there association between general health and alcohol drink?
```{r}
data3 <- df %>%
...
ggplot(data3, aes(x= genhlth, y = avedrnk2)) + geom_point(aes(x= genhlth, y = avg_drink)) + labs(x = "general health", y = "average drink", title = "general health vs average drink")
```
```{r}
p2 <- data3 %>%
  ggplot() + geom_col(aes(x = genhlth, y = avedrnk2, fill = sex), position = "dodge") + xlab("general health") +
  ggtitle("general health vs avg sleep") 
